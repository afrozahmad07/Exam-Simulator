...
if [ ! -f ".migration_user_answer_done" ]; then
    ...
    AUTO_MIGRATE=true python3 migrate_user_answer_column.py && touch .migration_user_answer_done || print_warning "User answer migration failed or already applied"
else
...
if [ ! -f ".migration_url_path_done" ]; then
    ...
    AUTO_MIGRATE=true python3 migrate_url_path_constraint.py && touch .migration_url_path_done || print_warning "URL path migration failed or already applied"
else
